...
func discover(t *testing.T, sources map[string]string, scan ...string) ([]*Operation, error) {
	...
	req := golden.Request(t, importPaths, sources, "", scan...)
	...
	for _, name := range scan {
		...
		for i := 0; i < fd.Services().Len(); i++ {
			o, e := discoverService(fd.Services().Get(i))
			ops, errs = append(ops, o...), append(errs, e...)
		}
	}
	errs = append(errs, duplicates{}.check(ops)...)
	if len(errs) > 0 {
