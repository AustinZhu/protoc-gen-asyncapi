...
func TestGolden(t *testing.T) {
	cases := []struct {
		...
		{"overrides", "", []string{"overrides/v1/overrides.proto"}},
...
`
	cases := []struct {
		...
		{"payload schema not JSON", "", `message Ev { option (nats.asyncapi.v1.message) = {subject: "ev"}; option (asyncapi.v3.message) = {payload_schema: "{type: string}"}; }`,
			"test.proto:7:1: message test.v1.Ev: payload_schema is not valid JSON"},
		{"payload schema not object", "", `message Ev { option (nats.asyncapi.v1.message) = {subject: "ev"}; option (asyncapi.v3.message) = {payload_schema: "[1]"}; }`,
			"payload_schema must be a JSON object"},
		{"payload schema invalid", "", `message Ev { option (nats.asyncapi.v1.message) = {subject: "ev"}; option (asyncapi.v3.message) = {payload_schema: '{"type": "bytes"}'}; }`,
			"payload_schema is not a valid JSON Schema: at /type"},
		{"payload format without schema", "", `message Ev { option (nats.asyncapi.v1.message) = {subject: "ev"}; option (asyncapi.v3.message) = {payload_schema_format: "application/vnd.apache.avro+json;version=1.9.0"}; }`,
			"payload_schema_format requires payload_schema"},
		{"payload format with invalid JSON", "", `message Ev { option (nats.asyncapi.v1.message) = {subject: "ev"}; option (asyncapi.v3.message) = {payload_schema_format: "x", payload_schema: "nope"}; }`,
			"payload_schema is not valid JSON"},
		{"correlation field with payload schema", "", `message Ev { option (nats.asyncapi.v1.message) = {subject: "ev"}; option (asyncapi.v3.message) = {payload_schema: '{"type": "string"}'}; string id = 1 [(asyncapi.v3.field) = {correlation_id: true}]; }`,
			"correlation_id: the payload of Ev is replaced by payload_schema"},
		{"field schema not JSON", "", `message Ev { option (nats.asyncapi.v1.message) = {subject: "ev"}; uint64 at = 1 [(asyncapi.v3.field) = {schema: "integer"}]; }`,
			"test.proto:7:67: field test.v1.Ev.at: schema is not valid JSON"},
		{"field schema invalid", "", `message Ev { option (nats.asyncapi.v1.message) = {subject: "ev"}; uint64 at = 1 [(asyncapi.v3.field) = {schema: '{"minimum": "zero"}'}]; }`,
			"schema is not a valid JSON Schema: at /minimum"},
		{"field schema dangling ref", "", `message Ev { option (nats.asyncapi.v1.message) = {subject: "ev"}; uint64 at = 1 [(asyncapi.v3.field) = {schema: '{"$ref": "#/components/schemas/Nope"}'}]; }`,
