...
func compile(t *testing.T) (protoreflect.FileDescriptor, *dynamicpb.Types) {
	t.Helper()
	files, registry := filesFrom(t, map[string]string{"t.proto": source}, "t.proto")
	return files[0], dynamicpb.NewTypes(registry)
}
...
func prop(t *testing.T, s *asyncapi.Schema, name string) *asyncapi.Schema {
	t.Helper()
...
func TestFieldMapping(t *testing.T) {
	fd, _ := compile(t)
	m, defs := MessageSchema(fd.Messages().ByName("M"), SchemaOptions{})
...
func TestUseProtoNames(t *testing.T) {
	fd, _ := compile(t)
	m, _ := MessageSchema(fd.Messages().ByName("N"), SchemaOptions{UseProtoNames: true})
	m2, _ := MessageSchema(fd.Messages().ByName("M"), SchemaOptions{UseProtoNames: true})
...
func TestProtovalidate(t *testing.T) {
	...
	off, _ := MessageSchema(md, SchemaOptions{})
	...
	on, _ := MessageSchema(md, SchemaOptions{Protovalidate: types})
	tests := map[string]string{
...
func keys(m map[string]*asyncapi.Schema) []string {
	var out []string
	for k := range m {
		out = append(out, k)
	}
