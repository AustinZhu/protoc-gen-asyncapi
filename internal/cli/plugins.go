...
// several. Temporal, Redis, AMQP and Google Pub/Sub claim first: their
// services are explicitly annotated, while NATS also infers services.
var All = core.Plugin{
	Name: "protoc-gen-asyncapi",
	Protocols: func() []core.Protocol {
		return []core.Protocol{temporal.New(), redis.New(), amqp.New(), googlepubsub.New(), nats.New()}
	},
}
